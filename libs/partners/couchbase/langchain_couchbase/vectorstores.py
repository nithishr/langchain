...
class CouchbaseSimilarityMetric(Enum):
    DOT = "dot"
    L2 = "l2"
    EUCLIDEAN = "euclidean"
    COSINE = "cosine"
...
class CouchbaseGSIVectorStore(VectorStore):
    # Default batch size
    DEFAULT_BATCH_SIZE = 100
    _metadata_key = "metadata"
    _default_text_key = "text"
    ...
    def __init__(
        ...
        collection_name: str,
        embedding: Embeddings,
        similarity_metric: CouchbaseSimilarityMetric,
        *,
        text_key: Optional[str] = _default_text_key,
        embedding_key: Optional[str] = _default_embedding_key,
    ) -> None:
        """
        Initialize the Couchbase GSI Vector Store.
        ...
        if not isinstance(similarity_metric, CouchbaseSimilarityMetric):
            raise ValueError(
                "similarity_metric should be an instance of CouchbaseSimilarityMetric, "
                f"got {type(similarity_metric)}"
            )
        ...
        self._text_key = text_key
        self._embedding_key = embedding_key
        self._similarity_metric = similarity_metric.value
    ...
    def similarity_search_with_score_by_vector(
        ...
        **kwargs: Any,
        ...
        similarity_search_string = (
            f"ANN({self._embedding_key}, {embedding}, '{self._similarity_metric}')"
        )

        if not where_str:
            search_query = (
                f"SELECT META().id, {','.join(fields)}, "
                f"{similarity_search_string} as score "
        ...
        # print(f"{self._similarity_metric}")
    ...
    @classmethod
    def _from_kwargs(
        cls: Type[CouchbaseGSIVectorStore],
        embedding: Embeddings,
        **kwargs: Any,
    ) -> CouchbaseGSIVectorStore:
        """Initialize the Couchbase GSI vector store from keyword arguments for the
        vector store.
        ...
        collection_name = kwargs.get("collection_name", None)
        text_key = kwargs.get("text_key", cls._default_text_key)
        similarity_metric = kwargs.get("similarity_metric", None)
        embedding_key = kwargs.get("embedding_key", cls._default_embedding_key)
